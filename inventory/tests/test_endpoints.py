...
def test_ChangeLocationView(
    payload, expected_status, test_component, test_location, test_location2, test_user, test_released_component
):
    client = APIClient()
    ...
    if expected_status == status.HTTP_200_OK:
        assert test_location2.components.filter(unique_code="unique_code_1").exists()
...
def test_ChangeLocationView_max_weight(test_location2, test_component, test_user):
    ...
    heavy_component = Component.objects.create(
        code="test_code",
    ...
    body = {
        "unique_code": heavy_component.unique_code,
        "location": test_location2.name,
    }
...
def test_ReleasedComponentView(payload, expected_status, test_component, test_released_component, test_user):
    ...
    if expected_status == status.HTTP_201_CREATED:
        ...
        assert ReleasedComponent.objects.filter(
            unique_code=payload["unique_code"], department=payload["department"]
        ).exists()
...
def test_CheckLocationView(location, expected_status, test_location, test_user):
    ...
    if expected_status == status.HTTP_200_OK:
        ...
        assert (
            second_element["code"] == "15016610"
            and second_element["total_quantity"] == component_1.quantity + component_2.quantity
        )
...
def test_CheckComponentView(code, expected_status, test_location, test_location2, test_user):
    ...
    if expected_status == status.HTTP_200_OK:
        ...
        assert component_2["unique_code"] == younger_component.unique_code
...
def test_UndoComponentView(payload, expected_status, test_user, test_released_component, test_location):
    ...
    if expected_status == status.HTTP_201_CREATED:
        ...
        component = Component.objects.get(unique_code=test_released_component.unique_code)
...
def test_UndoComponentView_max_weight(test_user, test_location, test_released_component):
    """
    In this test we assign a component with 790 kg weight to our location
    So when we try undo our component to this location the endpoint should not allow us to do that
    because location will be too heavy
    """
    ...
    assert response.data["message"] == (
        f'You can"t add component {test_released_component.code} to location {test_location.name}'
